...
from NeuroFlex.advanced_models.advanced_time_series_analysis import (
...
def test_arima_forecast(time_series_analyzer, sample_data):
    ...
    with pytest.raises(
        ValueError, match="Input data must be numeric for ARIMA forecast"
    ):
        time_series_analyzer.arima_forecast(pd.Series(["a", "b", "c"]))

    # Test with DataFrame missing 'y' column
    with pytest.raises(
        ValueError, match="DataFrame must contain a 'y' column for ARIMA forecast"
    ):
...
def test_sarima_forecast(time_series_analyzer, sample_data):
    ...
    # Test with non-numeric data
    with pytest.raises(
        ValueError, match="Input data must be numeric for SARIMA forecast"
    ):
        time_series_analyzer.sarima_forecast(pd.Series(["a", "b", "c"]))

    # Test with DataFrame missing 'y' column
    with pytest.raises(
        ValueError, match="DataFrame must contain a 'y' column for SARIMA forecast"
    ):
...
@pytest.mark.parametrize(
    "method, exception, error_message, test_data",
    [
        (
            "arima",
            ValueError,
            "DataFrame must contain a 'y' column for ARIMA forecast",
            pd.DataFrame({"x": [1, 2, 3]}),
        ),
        (
            "arima",
            ValueError,
            "Input data must be numeric for ARIMA forecast",
            pd.DataFrame({"y": ["a", "b", "c"]}),
        ),
        (
            "sarima",
            ValueError,
            "DataFrame must contain a 'y' column for SARIMA forecast",
            pd.DataFrame({"x": [1, 2, 3]}),
        ),
        (
            "sarima",
            ValueError,
            "Input data must be numeric for SARIMA forecast",
            pd.DataFrame({"y": ["a", "b", "c"]}),
        ),
        (
            "prophet",
            ValueError,
            "Invalid DataFrame for Prophet. It must have columns 'ds' and 'y' with the dates and values respectively.",
            pd.DataFrame({"x": [1, 2, 3]}),
        ),
    ],
)
def test_analyze_exceptions(
    time_series_analyzer, method, exception, error_message, test_data
):
...
@pytest.mark.parametrize(
    "method, warning_message, data, order, seasonal_order",
    [
        (
            "arima",
            "Non-stationary starting autoregressive parameters found. Using zeros as starting parameters.",
            pd.Series(np.exp(np.linspace(0, 5, 100))),
            (1, 1, 1),
            None,
        ),  # Exponential growth series for ARIMA
        (
            "sarima",
            "Non-invertible starting MA parameters found. Using zeros as starting parameters.",
            pd.Series(
                np.random.normal(0, 1, 200)
                + 10 * np.sin(np.linspace(0, 4 * np.pi, 200))
            ),
            (1, 1, 2),
            (1, 1, 1, 12),
        ),  # Seasonal data with strong MA component
    ],
)
def test_analyze_warnings(
    time_series_analyzer, method, warning_message, data, order, seasonal_order
):
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        result = time_series_analyzer.analyze(
            method, data, order=order, seasonal_order=seasonal_order
        )
        ...
        if method == "sarima":
            assert any(
                ...
                and warning_message in str(warn.message)
                for warn in w
            ), f"Expected UserWarning with message '{warning_message}' not found"
        else:
            assert any(
                warning_message in str(warn.message) for warn in w
...
def test_update_performance(time_series_analyzer):
    ...
    time_series_analyzer._update_performance(
        {"forecast": pd.Series([1, 2, 3]), "actual": pd.Series([1.1, 2.1, 3.1])}
    )
...
@pytest.mark.parametrize(
    "method, order, seasonal_order, data",
    [
        ...
        (
            ...
            (1, 1, 1, 12),
            pd.Series(
                np.random.randn(200) + 10 * np.sin(np.linspace(0, 4 * np.pi, 200))
            ),
        ),
        (
            "sarima",
            ...
            (0, 1, 1, 4),
            pd.Series(
                np.random.randn(100) + 5 * np.sin(np.linspace(0, 8 * np.pi, 100))
            ),
        ),
    ],
)
def test_analyze_different_configurations(
    time_series_analyzer, method, order, seasonal_order, data
):
    kwargs = {"order": order}
    if seasonal_order:
...
def test_analyze_edge_cases(time_series_analyzer, method, data):
    with warnings.catch_warnings(record=True) as w:
        ...
        if len(w) > 0:
            logger.info(
                f"Warnings raised for {method} with data type {type(data)}: {[str(warn.message) for warn in w]}"
            )
