...
from NeuroFlex.core_neural_networks import LSTMModule
...
class TestLSTM(unittest.TestCase):
    ...
    def test_lstm_module_forward_pass(self):
        ...
        variables = lstm.init(self.rng, x)
        output, (final_h, final_c) = lstm.apply(variables, x, rngs={'dropout': self.rng})

        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.hidden_size))
        self.assertEqual(final_h.shape, (self.num_layers, self.batch_size, self.hidden_size))
        self.assertEqual(final_c.shape, (self.num_layers, self.batch_size, self.hidden_size))
    ...
    def test_lstm_module_dropout(self):
        ...
        variables_with_dropout = lstm_with_dropout.init(self.rng, x)
        variables_without_dropout = lstm_without_dropout.init(self.rng, x)

        output_with_dropout, _ = lstm_with_dropout.apply(variables_with_dropout, x, rngs={'dropout': self.rng}, train=True)
        output_without_dropout, _ = lstm_without_dropout.apply(variables_without_dropout, x, rngs={'dropout': self.rng}, train=True)
        ...
        output_with_dropout_eval, _ = lstm_with_dropout.apply(variables_with_dropout, x, rngs={'dropout': self.rng}, train=False)
        output_without_dropout_eval, _ = lstm_without_dropout.apply(variables_without_dropout, x, rngs={'dropout': self.rng}, train=False)
