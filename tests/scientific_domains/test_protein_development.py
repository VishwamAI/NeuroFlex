...
class TestProteinDevelopment(unittest.TestCase):
    ...
    @patch(
        "NeuroFlex.scientific_domains.protein_development.data.get_model_haiku_params"
    )
    ...
    def test_setup_alphafold(
        self, mock_prng_key, mock_run_model, mock_get_params, mock_model_config
    ):
        ...
        mock_run_model.assert_called_once_with(
            mock_model_config.return_value, mock_get_params.return_value
        )
    ...
    @patch(
        "NeuroFlex.scientific_domains.protein_development.pipeline.make_sequence_features"
    )
    ...
    def test_predict_structure(self, mock_from_prediction, mock_make_sequence_features):
        ...
        self.assertEqual(
            result["predicted_tm_score"], mock_prediction["predicted_tm_score"]
        )
        ...
        self.protein_dev.alphafold_model.predict.side_effect = Exception(
            "Prediction error"
        )
        with self.assertRaises(RuntimeError) as context:
            self.protein_dev.predict_structure(sequence)
    ...
    @patch(
        "NeuroFlex.scientific_domains.protein_development.openmm.LangevinMiddleIntegrator"
    )
    ...
    def test_setup_openmm_simulation(
        self,
        mock_modeller,
        mock_simulation,
        mock_integrator,
        ...
        mock_protein_structure.atom_positions = [
            [[1, 1, 1], [2, 2, 2]],
            [[3, 3, 3], [4, 4, 4]],
            [[5, 5, 5], [6, 6, 6]],
        ]
        ...
        mock_forcefield.assert_called_once_with(
        ...
        expected_atom_count = sum(
            sum(mask) for mask in mock_protein_structure.atom_mask
        )
        self.assertEqual(
            mock_topology.return_value.addAtom.call_count, expected_atom_count
        )
    ...
    def test_run_molecular_dynamics(self):
        ...
        self.protein_dev.openmm_simulation.context.setVelocitiesToTemperature.assert_called_once_with(
            300 * unit.kelvin
        )
        assert self.protein_dev.openmm_simulation.step.call_count == 2
        self.protein_dev.openmm_simulation.step.assert_any_call(
            1000
        )  # Equilibration step
        self.protein_dev.openmm_simulation.step.assert_any_call(
            1000
