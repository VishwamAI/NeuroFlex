...
def test_transformer_forward_pass(unified_transformer):
    ...
    assert (
        output.shape[2] == unified_transformer.d_model
    )  # Changed from hidden_size to d_model
...
def test_transformer_task_specific_forward(unified_transformer):
    ...
    unified_transformer.fine_tune(task="classification", num_labels=3)
    classification_output = unified_transformer.task_specific_forward(
        input_ids, attention_mask, task="classification"
    )
    assert classification_output.shape == (batch_size, 3)

    unified_transformer.fine_tune(task="generation")
    generation_output = unified_transformer.task_specific_forward(
        input_ids, attention_mask, task="generation"
    )
    assert generation_output.shape == (
        batch_size,
        seq_length,
        unified_transformer.vocab_size,
    )
