...
class TestMathSolver(unittest.TestCase):
    ...
    def test_numerical_optimization(self):
        ...
        result = self.math_solver.numerical_optimization(func, [0, 0])
    ...
    def test_linear_algebra_operations(self):
        ...
        result_multiply = self.math_solver.linear_algebra_operations(
            matrix_a, matrix_b, operation="multiply"
        )
        expected_multiply = np.array([[19, 22], [43, 50]])
        np.testing.assert_array_almost_equal(result_multiply, expected_multiply)

        # Test matrix inverse
        result_inverse = self.math_solver.linear_algebra_operations(
            matrix_a, None, operation="inverse"
        )
        expected_inverse = np.array([[-2, 1], [1.5, -0.5]])
        np.testing.assert_array_almost_equal(result_inverse, expected_inverse)

        # Test eigenvalues
        result_eigenvalues = self.math_solver.linear_algebra_operations(
            matrix_a, None, operation="eigenvalues"
        )
        ...
        with self.assertRaises(ValueError):
            self.math_solver.linear_algebra_operations(
                matrix_a, matrix_b, operation="unsupported"
            )
