...
def test_preprocess(bci_processor, sample_eeg_data, sample_labels):
    ...
    assert sample_eeg_data.ndim == 3, "Input data should be 3D (trials x channels x samples)"
    ...
    assert preprocessed_data.shape[0] == n_trials, "Number of trials should be preserved"
    assert preprocessed_data.shape[1] <= n_channels, "CSP may reduce dimensionality"
    assert preprocessed_data.shape != sample_eeg_data.shape, "Data should be transformed"
...
def test_process(bci_processor, sample_eeg_data, sample_labels):
    ...
    for feature_name, feature_data in processed_data.items():
        if 'wavelet' in feature_name:
            # For wavelet features, the shape should be (n_channels, n_wavelet_coeffs)
            assert feature_data.shape[0] == n_channels, f"Expected {n_channels} channels, but got {feature_data.shape[0]} for {feature_name}"
        else:
            # For power features, the shape should be (n_channels, n_freq_bins)
            assert feature_data.shape[0] == n_channels, f"Expected {n_channels} channels, but got {feature_data.shape[0]} for {feature_name}"
            assert feature_data.shape[1] > 0, f"Expected non-zero frequency bins, but got {feature_data.shape[1]} for {feature_name}"
        print(f"Shape of {feature_name}: {feature_data.shape}")  # Debug print
