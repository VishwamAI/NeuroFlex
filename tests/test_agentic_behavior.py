...
from NeuroFlex.Prompt_Agent.agentic_behavior import (
    ZeroShotAgent,
    FewShotAgent,
    ChainOfThoughtAgent,
    MetaPromptingAgent,
...
class TestZeroShotAgent(unittest.TestCase):
    ...
    def test_self_correct(self):
        ...
        expected_encoded_input = jnp.array(
            [hash(token) % 10000 for token in expected_tokens]
        )

        with patch.object(
            self.agent, "_encode_input", return_value=expected_encoded_input
        ) as mock_encode:
            with patch.object(
                self.agent.model, "apply", return_value=jnp.array([16.0, 17.0, 18.0])
            ) as mock_apply:
                with patch.object(
                    self.agent,
                    "_decode_output",
                    ...
                    mock_apply.assert_called_once_with(
                        {"params": self.mock_model.params}, expected_encoded_input
                    )
                    self.assertTrue(
                        jnp.allclose(
                            mock_decode.call_args[0][0], jnp.array([16.0, 17.0, 18.0])
                        )
                    )
    ...
    def test_self_update(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_once_with(
                f"Received feedback for self-update: {feedback}"
            )
    ...
    def test_zero_shot(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.agent.model, "apply", return_value=jnp.array([1.0, 2.0, 3.0])
            ) as mock_apply:
                with patch.object(
                    self.agent, "_decode_output", return_value="Bonjour"
                ) as mock_decode:
                    ...
                    self.assertTrue(
                        jnp.allclose(
                            mock_decode.call_args[0][0], jnp.array([1.0, 2.0, 3.0])
                        )
                    )
...
class TestFewShotAgent(unittest.TestCase):
    ...
    def test_few_shot(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.agent.model, "apply", return_value=jnp.array([4.0, 5.0, 6.0])
            ) as mock_apply:
                with patch.object(
                    self.agent, "_decode_output", return_value="Buenos días"
                ) as mock_decode:
                    ...
                    self.assertTrue(
                        jnp.allclose(
                            mock_decode.call_args[0][0], jnp.array([4.0, 5.0, 6.0])
                        )
                    )
...
class TestChainOfThoughtAgent(unittest.TestCase):
    ...
    def test_chain_of_thought(self):
        ...
        initial_encoded_input = jnp.array(
            [hash(token) % 10000 for token in initial_tokens]
        )
        ...
        with patch.object(
            self.agent, "_encode_input", side_effect=encode_side_effects
        ) as mock_encode:
            with patch.object(
                self.agent.model, "apply", side_effect=apply_side_effects
            ) as mock_apply:
                with patch.object(
                    self.agent, "_decode_output", side_effect=decode_side_effects
                ) as mock_decode:
                    ...
                    mock_encode.assert_has_calls(
                        [
                            ...
                            *[
                                unittest.mock.call(tokenize_text(step))
                                for step in decode_side_effects
                            ],
                        ]
                    )
                    ...
                    mock_apply.assert_has_calls(
                        [
                            unittest.mock.call(
                                {"params": self.mock_model.params}, unittest.mock.ANY
                            )
                            for _ in range(5)
                        ]
                    )
                    ...
                    for i, array in enumerate(apply_side_effects):
                        self.assertTrue(
                            jnp.allclose(mock_decode.call_args_list[i][0][0], array)
                        )
...
class TestMetaPromptingAgent(unittest.TestCase):
    ...
    def test_meta_prompting(self):
        ...
        meta_prompt = (
            "You are a helpful AI assistant specializing in language translation."
        )
        ...
        with patch.object(
            ...
            with patch.object(
                self.agent.model, "apply", return_value=jnp.array([13.0, 14.0, 15.0])
            ) as mock_apply:
                with patch.object(
                    self.agent, "_decode_output", return_value="こんにちは"
                ) as mock_decode:
                    ...
                    self.assertTrue(
                        jnp.allclose(
                            mock_decode.call_args[0][0], jnp.array([13.0, 14.0, 15.0])
                        )
                    )
...
class TestSelfConsistencyAgent(unittest.TestCase):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    self.agent,
                    "_decode_output",
                    side_effect=["Paris", "London", "Paris"],
                ) as mock_decode:
    ...
    def test_zero_shot(self):
        prompt = "What is the capital of France?"
        with patch.object(
            self.agent, "generate_samples", return_value=["Paris", "London", "Paris"]
        ) as mock_generate:
            with patch.object(
                self.agent, "select_most_consistent", return_value="Paris"
            ) as mock_select:
                result = self.agent.zero_shot(prompt)
...
class TestGenerateKnowledgePromptingAgent(unittest.TestCase):
    def setUp(self):
        ...
        self.agent = GenerateKnowledgePromptingAgent(
            self.mock_model, self.knowledge_base
        )
    ...
    def test_generate_knowledge(self):
        ...
        self.assertEqual(
            knowledge,
            "France is a country in Western Europe with a capital city of Paris.",
        )
    ...
    def test_zero_shot(self):
        ...
        expected_tokens = tokenize_text(
            "Given the following knowledge: France is a country in Western Europe with a capital city of Paris.\n\nAnswer the question: What is the capital of France?"
        )
        expected_encoded_input = jnp.array(
            [hash(token) % 10000 for token in expected_tokens]
        )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    self.agent, "_decode_output", return_value="Paris"
                ) as mock_decode:
                    result = self.agent.zero_shot(prompt)
