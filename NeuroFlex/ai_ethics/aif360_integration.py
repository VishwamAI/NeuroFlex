...
class AIF360Integration:
    ...
    def load_dataset(
        ...
        self.privileged_groups = [
            {protected_attribute_names[i]: privileged_classes[i][0]}
            for i in range(len(protected_attribute_names))
        ]
        self.unprivileged_groups = [
            {protected_attribute_names[i]: privileged_classes[i][-1]}
            for i in range(len(protected_attribute_names))
        ]
    ...
    def mitigate_bias(self, method="reweighing"):
        ...
        if method == "reweighing":
            ...
            PR = PrejudiceRemover(
                sensitive_attr=self.dataset.protected_attribute_names[0], eta=25.0
            )
