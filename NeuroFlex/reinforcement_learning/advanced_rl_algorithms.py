...
class SACAgent:
    ...
    def __init__(
        ...
        self.actor_params = self.actor.init(
            jax.random.PRNGKey(0), jnp.zeros((1, state_dim))
        )
        self.critic1_params = self.critic1.init(
            jax.random.PRNGKey(1), jnp.zeros((1, state_dim)), jnp.zeros((1, action_dim))
        )
        self.critic2_params = self.critic2.init(
            jax.random.PRNGKey(2), jnp.zeros((1, state_dim)), jnp.zeros((1, action_dim))
        )
    ...
    def update(self, batch: Tuple[jnp.ndarray, ...]) -> Tuple[float, float]:
        ...
        target_q1 = self.target_critic1.apply(
            self.target_critic1_params, next_states, next_actions
        )
        target_q2 = self.target_critic2.apply(
            self.target_critic2_params, next_states, next_actions
        )
        ...
        critic_loss, critic1_grads = jax.value_and_grad(critic_loss_fn)(
            self.critic1_params
        )
        critic_loss, critic2_grads = jax.value_and_grad(critic_loss_fn)(
            self.critic2_params
        )
        self.critic1_params, self.critic1_opt_state = self.critic_optimizer.update(
            critic1_grads, self.critic1_opt_state, self.critic1_params
        )
        self.critic2_params, self.critic2_opt_state = self.critic_optimizer.update(
            critic2_grads, self.critic2_opt_state, self.critic2_params
        )
        ...
        self.actor_params, self.actor_opt_state = self.actor_optimizer.update(
            actor_grads, self.actor_opt_state, self.actor_params
        )

        # Update target networks
        self.target_critic1_params = self.soft_update(
            self.critic1_params, self.target_critic1_params
        )
        self.target_critic2_params = self.soft_update(
            self.critic2_params, self.target_critic2_params
        )
    ...
    def soft_update(self, source_params: dict, target_params: dict) -> dict:
        ...
        return jax.tree_map(
            lambda s, t: self.tau * s + (1 - self.tau) * t, source_params, target_params
        )
...
class TD3Agent:
    ...
    def update(self, batch: Tuple[jnp.ndarray, ...], step: int) -> Tuple[float, float]:
        ...
        noise = jnp.clip(
            jax.random.normal(jax.random.PRNGKey(step), shape=actions.shape)
            * self.policy_noise,
            -self.noise_clip,
            self.noise_clip,
        )
        next_actions = jnp.clip(
            self.actor.apply(self.target_actor_params, next_states) + noise, -1, 1
        )

        # Compute target Q-values
        target_q1 = self.critic1.apply(
            self.target_critic1_params, next_states, next_actions
        )
        target_q2 = self.critic2.apply(
            self.target_critic2_params, next_states, next_actions
        )
        target_q = jnp.minimum(target_q1, target_q2)
        target_q = rewards + (1 - dones) * self.gamma * target_q
        ...
        )(self.critic1_params, self.critic2_params)
        self.critic1_params, self.critic1_opt_state = self.critic_optimizer.update(
            critic1_grads, self.critic1_opt_state, self.critic1_params
        )
        self.critic2_params, self.critic2_opt_state = self.critic_optimizer.update(
        ...
        if step % self.policy_freq == 0:
            ...
            def actor_loss_fn(actor_params):
                actions = self.actor.apply(actor_params, states)
                actor_loss = -jnp.mean(
                    self.critic1.apply(self.critic1_params, states, actions)
                )
                return actor_loss
            ...
            # Update target networks
            self.target_actor_params = self.soft_update(
