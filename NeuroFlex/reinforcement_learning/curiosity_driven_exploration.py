...
class ICM(nn.Module):
    ...
    def forward(self, state, next_state, action):
        ...
        print(
            f"state_feat shape: {state_feat.shape}, action_one_hot shape: {action_one_hot.shape}"
        )
        if action_one_hot.size(1) != self.action_dim:
            raise ValueError(
                f"Action one-hot encoding size mismatch. Expected {self.action_dim}, got {action_one_hot.size(1)}"
            )
        ...
        if current_input_size != expected_input_size:
            raise ValueError(
                f"Input size mismatch. Expected {expected_input_size}, got {current_input_size}"
            )

        print(f"Final input size: {state_feat.size(1) + action_one_hot.size(1)}")

        pred_next_state_feat = self.forward_model(
            torch.cat([state_feat, action_one_hot], dim=1)
        )
        ...
        if pred_action.size(1) != self.action_dim:
            raise ValueError(
                f"Predicted action size mismatch. Expected {self.action_dim}, got {pred_action.size(1)}"
            )
    ...
    def compute_intrinsic_reward(self, state, next_state, action):
        with torch.no_grad():
            ...
            intrinsic_reward = (
                self.beta
                * 0.5
                * torch.mean((pred_next_state_feat - next_state_feat) ** 2, dim=1)
            )
        return (
            intrinsic_reward.item()
            if intrinsic_reward.numel() == 1
            else intrinsic_reward.mean().item()
        )

    def update(self, state, next_state, action):
        pred_action, pred_next_state_feat, next_state_feat = self(
        ...
        forward_loss = 0.5 * torch.mean(
            (pred_next_state_feat - next_state_feat.detach()) ** 2, dim=1
        )
...
class NoveltyDetector:
    ...
    def compute_novelty(self, state):
        if self.memory_index < self.memory_size:
            distances = np.mean(
                np.abs(self.memory[: self.memory_index] - state), axis=1
            )
        else:
...
class CuriosityDrivenAgent:
    ...
    def act(self, state):
        ...
        state = torch.FloatTensor(state_np).unsqueeze(
            0
        )  # Convert to numpy array, then to tensor, and add batch dimension
        # print(f"Processed state type: {type(state)}, shape: {state.shape}")
        action_probs = self.actor(state)
        action = torch.argmax(action_probs, dim=1)
        action_one_hot = F.one_hot(
            action, num_classes=self.icm.action_dim
        ).float()  # Use self.icm.action_dim for correct size
        return (
            action.item(),
            action_one_hot.squeeze(0).detach().numpy(),
        )  # Return both scalar action and numpy array of one-hot encoded action
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        icm_loss = self.icm.update(
            state.unsqueeze(0), next_state.unsqueeze(0), action.unsqueeze(0)
        )
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            total_reward = agent.compute_total_reward(
                state, next_state, action_one_hot, extrinsic_reward
            )
