...
def train_conv_model(
    ...
    for epoch in range(epochs):
        ...
        if epoch > 0 and history["train_loss"][-1] > history["train_loss"][-2]:
            logging.info(
                f"Performance degradation detected at epoch {epoch}. Adjusting learning rate."
            )
...
def monitor_performance(
    model: tf.keras.Model, x_test: tf.Tensor, y_test: tf.Tensor, threshold: float = 0.8
):
    test_loss = tf.keras.losses.MeanSquaredError()
    predictions = model(x_test, training=False)
    current_performance = (
        1 - test_loss(y_test, predictions).numpy()
    )  # Assuming lower loss is better

    if current_performance < threshold:
        logging.warning(
            f"Model performance ({current_performance:.4f}) is below threshold ({threshold})."
        )
        return False
    return True
